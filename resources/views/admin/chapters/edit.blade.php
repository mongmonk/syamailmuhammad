...
<div class="bg-white py-8">
  <div class="max-w-5xl mx-auto px-4">
    ...
    <div class="flex items-start justify-between mb-3">
      <div>
        <h1 class="text-2xl font-bold">Edit Bab</h1>
        <p class="text-sm text-gray-600">Perbarui informasi bab secara rapi dan konsisten.</p>
      </div>
          ...
          <div class="px-6 py-4 border-b">
            <div class="flex items-center justify-between">
              <div class="text-sm text-gray-600">
                <span class="mr-4">ID: <span class="font-medium text-gray-800">{{ $chapter->id }}</span></span>
                <span>Bab saat ini: <span class="font-medium text-gray-800">{{ $chapter->chapter_number }}</span></span>
              </div>
            </div>
          </div>

          <form method="POST" action="{{ route('admin.chapters.update', $chapter->id) }}" class="px-6 py-6 space-y-6">
            @csrf
            @method('PATCH')
