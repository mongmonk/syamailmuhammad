...
class UpdateHadithRequest extends FormRequest
{
    ...
    /**
     * Aturan validasi untuk pembaruan Hadits.
     */
    public function rules(): array
    {
        $chapterId = $this->input('chapter_id');

        // Ambil ID hadits dari route model binding atau parameter
        $routeHadith = $this->route('hadith');
        $hadithId = is_object($routeHadith) ? ($routeHadith->id ?? null) : $routeHadith;
        if (!$hadithId) {
            // fallback umum: coba 'id' dari route param atau body
            $hadithId = $this->route('id') ?? $this->input('id');
        }
        ...
        return [
            ...
            'hadith_number' => [
                ...
                'integer',
                'min:1',
                Rule::unique('hadiths', 'hadith_number')
